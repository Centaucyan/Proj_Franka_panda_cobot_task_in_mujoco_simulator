...
│       └── meshes/
└── test/
    ├── test_phase02_scene.py                       # [Step 4] 씬 로딩 및 무결성 검증 스크립트
    └── view_phase02_scene.py                       # [Step 5] 3D 대화형 뷰어 실행 스크립트
```
  ...
  <worldbody>
    ...
    <body name="r2_link0" pos="0.65 0 0.75" childclass="panda">
      ...
      <body name="r2_link1" pos="0 0 0.333">
        ...
        <body name="r2_link2" quat="0.707107 -0.707107 0 0">
          <inertial mass="0.646926" pos="-0.003141 -0.02872 0.003495"
            fullinertia="0.0079620 0.028350 0.028403 -0.003925 0.001024 -0.000803"/>
          ...
          <body name="r2_link3" pos="0 -0.316 0" quat="0.707107 0.707107 0 0">
            <inertial mass="3.228604" pos="0.027518 0.039252 -0.066502"
              fullinertia="0.037242 0.036155 0.01083 -0.004761 -0.011396 -0.012805"/>
            ...
            <body name="r2_link4" pos="0.0825 0 0" quat="0.707107 0.707107 0 0">
              <inertial mass="3.587895" pos="-0.05317 0.104419 0.027454"
                fullinertia="0.025853 0.019552 0.028323 0.007796 -0.001332 0.008641"/>
              ...
              <body name="r2_link5" pos="-0.0825 0.384 0" quat="0.707107 -0.707107 0 0">
                <inertial mass="1.225946" pos="-0.011953 0.041065 -0.038437"
                  fullinertia="0.035549 0.029474 0.008627 -0.002117 -0.004037 0.00229"/>
                ...
                <body name="r2_link6" quat="0.707107 0.707107 0 0">
                  <inertial mass="1.666555" pos="0.060149 -0.014117 -0.010517"
                    fullinertia="0.001964 0.004354 0.005433 0.000109 -0.001158 -0.000341"/>
                  ...
                  <body name="r2_link7" pos="0.088 0 0" quat="0.707107 0.707107 0 0">
                    <inertial mass="0.735522" pos="0.010517 -0.004252 0.061597"
                      fullinertia="0.012516 0.010027 0.004815 -0.000428 -0.001196 -0.000741"/>
...
### 🔹 Step 4: 씬 무결성 자가진단 스크립트 작성 (`test/test_phase02_scene.py`)

작성된 MJCF 파일이 MuJoCo 3.6.x C API에서 에러 없이 로드되는지, 모든 바디/관절/액추에이터/카메라가 정상 파싱되는지 검증하는 테스트 스크립트를 `test/test_phase02_scene.py`에 작성합니다.
...
### 🔹 Step 5: 대화형 시각화 스크립트 작성 (`test/view_phase02_scene.py`)
...
# 3. 자동 무결성 검증 실행
cd ..
python test/test_phase02_scene.py

# 4. (선택 사항) 3D 대화형 뷰어로 눈으로 직접 확인
python test/view_phase02_scene.py
```
...
- [ ] `python test/test_phase02_scene.py` 실행 시 6개 진단 항목이 모두 통과(Pass)하는가?
