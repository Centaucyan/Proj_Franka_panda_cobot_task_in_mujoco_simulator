...
* **재현성(Reproducibility)**: `environment.yaml`을 통해 팀원이나 다른 서버에서도 동일한 환경을 100% 재현
...
├── model/
|     └── franka_emika_panda/
├── environment.yaml                 # [Step 1] 가상환경 명세서
...
### 🔹 Step 1: Miniconda 가상환경 정의 및 생성 (`environment.yaml`)

프로젝트 루트 디렉토리(`Proj_Franka_panda_cobot_task_in_ros2_and_mujoco_simulator/`)에 `environment.yaml` 파일을 작성합니다.

#### 1) `environment.yaml` 파일 작성
```yaml
...
# 패키지 빌드 실행
colcon build --packages-select franka_logistics_msgs
...
> [!TIP]
> **`--symlink-install` 옵션을 제외한 이유**:
> Windows 파일 시스템(NTFS/exFAT) 또는 WSL 마운트 드라이브에서 `--symlink-install`을 사용할 경우, 심볼릭 링크 생성 권한 문제(`Operation not permitted`)나 파일 감지 오류로 빌드가 실패할 수 있습니다. 따라서 안전한 복사 기반의 기본 `colcon build` 명령어를 사용하는 것을 권장합니다.
